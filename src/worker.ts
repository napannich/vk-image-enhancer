...
import type { WorkerInput, WorkerOutput, EnhancementParams } from "./types.js";
...
async function processImage(
  ...
  const paramsOutput: WorkerOutput = {
    taskId,
    type: "progress",
    progress: 45,
    params: finalParams,
 ...
 */
function applyUnsharpMask(imageData: ImageData, strength: number): ImageData {
...
function sendProgress(taskId: string, progress: number): void {
  const output: WorkerOutput = { taskId, type: "progress", progress };
  self.postMessage(output);
}
